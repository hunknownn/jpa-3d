...
class ArchitectureDetectorTest {
    ...
    @Test fun singleModuleIsMonolith() {
        assertEquals(ArchitectureMode.MONOLITH, ArchitectureDetector.detect(1, 1, false))
        assertEquals(ArchitectureMode.MONOLITH, ArchitectureDetector.detect(0, 0, false))
    }

    @Test fun multipleBuildsIsMsa() {
        // 모듈 여럿 + settings.gradle 둘 이상 = 빌드/DB 분리 = MSA.
        assertEquals(ArchitectureMode.MSA, ArchitectureDetector.detect(3, 3, false))
        // 모듈 간 JPA 엣지가 있어도(경계 침범) 빌드가 갈렸으면 MSA 로 본다.
        assertEquals(ArchitectureMode.MSA, ArchitectureDetector.detect(3, 3, true))
    }

    @Test fun singleBuildMultipleModulesIsModularMonolith() {
        // 단일 settings.gradle + include(...) = 모듈러 모놀리스.
        assertEquals(ArchitectureMode.MODULAR_MONOLITH, ArchitectureDetector.detect(4, 1, false))
        assertEquals(ArchitectureMode.MODULAR_MONOLITH, ArchitectureDetector.detect(4, 1, true))
    }

    @Test fun unknownBuildFallsBackToJpaEdgeSignal() {
        // settings 못 읽음(0): 모듈 간 JPA 엣지 있으면 모듈러 모놀리스, 없으면 MSA.
        assertEquals(ArchitectureMode.MODULAR_MONOLITH, ArchitectureDetector.detect(2, 0, true))
        assertEquals(ArchitectureMode.MSA, ArchitectureDetector.detect(2, 0, false))
    }
